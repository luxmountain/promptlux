...
function EditTag({ selectedTags, setSelectedTags }) {
  ...
  useEffect(() => {
    ...
    const fetchTags = async () => {
      ...
      try {
        ...
        setTotalResults(data.matched_tags?.length || 0);
      } catch (error) {
  ...
  const selectTag = (tag) => {
    if (!selectedTags.some((t) => t.tid === tag.tid)) {
  ...
  return (
    <div className="relative w-full mt-1">
      ...
      {searchQuery && (
        <div
          className="absolute top-full mt-1 w-full bg-white rounded-lg shadow-lg max-h-[220px] overflow-y-auto border border-gray-300"
          aria-live="polite"
        >
          {loading ? (
            ...
            <p className="p-2 text-sm text-gray-500">No matching tags found</p>
          )}
        </div>
      )}
      ...
      <div className="flex flex-wrap gap-2 mt-4">
        {selectedTags.map((tag) => (
          <div
            key={tag.tid}
            className="flex items-center bg-black text-white px-3 py-2 rounded-2xl text-sm"
          >
            {tag}
            <button
